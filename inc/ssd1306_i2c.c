...
inline int ssd1306_get_font(uint8_t character)
{
    if (character >= 'A' && character <= 'Z') {
        ...
        return 37; // índice do ponto na sua fonte (ajuste conforme seu ssd1306_font.h)
    }
    else if (character == ',') {
        return 38; // índice da vírgula na sua fonte (ajuste conforme seu ssd1306_font.h)
    }
    else if (character == 0xB0 || character == 'o') {
        return 39; // índice do símbolo de grau na sua fonte (ajuste conforme seu ssd1306_font.h)
    }
    else
        return 0;
}
...
//Apaga o que está no display:
void ssd1306_clear() 
{
    uint8_t clear_buffer[ssd1306_width * (ssd1306_height / 8)];
    ...
    render_on_display(clear_buffer, &full_area);
}
